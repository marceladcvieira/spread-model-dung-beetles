...
CCapacity <- 'carrying capacity/o_alexis_carrying_capacity_wa_updated.tif'   # same as carrying_capacity_e.intermedius_per_gridcell_for_ca_model2.tif
ProbAdultSurvival <- 'prob survival/o_alexis_probability_of_survival_wa_updated.tif'

# should have the location and number of starting reproductive females (not the number of introduced individuals)
starting_colony_coords <- 'release data/o_alexis_by_six_months.csv'
...
EggToAdultSurvival <- 0.73      ### Tyndale-Biscoe 1988 table 1 at different temperatures
...
FlightDistance <- 2000 * 0.9 # Fincher et al. 1983  (using 1800m/time step after calibration)

# max number of offspring produced by each mating pair in a time interval
MaxNOffspring <- 125  # For O. alexis, we assumed eight weeks of reproduction (Tyndale-Biscoe, 1988) 
