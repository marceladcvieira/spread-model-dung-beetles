...
CCapacity <- 'carrying capacity/e_intermedius_carrying_capacity_wa.tif'   # same as carrying_capacity_e.intermedius_per_gridcell_for_ca_model2.tif
ProbAdultSurvival <- 'prob survival/e_intermedius_probability_of_survival_wa.tif'

# should have the location and number of starting reproductive females (not the number of introduced individuals)
starting_colony_coords <- 'release data/e_intermedius_by_six_months.csv'
...
EggToAdultSurvival <- 0.7      #Andrew & Holley (2019)
...
FlightDistance <- 2000 * 0.9 # Fincher at al. 1983  (using 1800m/time step after calibration)

# max number of offspring produced by each mating pair in a time interval
MaxNOffspring <- 111  # 111 Martínez et al.2019 
