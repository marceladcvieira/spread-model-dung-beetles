...
carrying_capacity_location <- 'o_alexis_carrying_capacity_wa_updated.tif'   # same as carrying_capacity_e.intermedius_per_gridcell_for_ca_model2.tif
probability_of_adult_survival_location <- 'o_alexis_probability_of_survival_wa_updated.tif'

# should have the location and number of starting reproductive females (not the number of introduced individuals)
starting_colony_coords <- 'Inputs120721/o_alexis_by_six_months.csv'
...
alateSurvival <- 0.73      ### Tyndale-Biscoe 1988 table 1 at different temperatures
                          ### Here we use 72% at temperature of 25 degrees celcius
...
meanFlightDist <- 2000 * 0.9 # Fincher et al. 1983  (using 1800m/time step after calibration)

# max number of offspring produced by each mating pair in a time interval
MaxNAlates <- 125  # For O. alexis, we assumed eight weeks of reproduction (Tyndale-Biscoe, 1988) 
                   # using the average number of two eggs produced per brood  mass (Edwards & Aschenborn, 1987; Tyndale-Biscoe, 1990) 
                   # and a total of 125 eggs/8 weeks (Fig 4a  Tyndale-Biscoe, 1988) or 125 eggs per time step


# max survival probability of a adult reproductive female
maxSurvival <- 0.9  # # # before it was 0.999 Theo Evans opinion to start with
